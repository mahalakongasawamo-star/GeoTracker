...
interface Mark {
  depth: number;
  /** Outer wrapper: positioning + size. NO opacity here — the entrance
   *  fade owns opacity on the wrapper, and resting opacity moves to the
   *  inner svg via `restOpacity` so the two never collide. */
  className: string;
  /** Inner svg ambient bob class (animate-idle-bob-*). */
  ambient: string;
  /** Inline opacity the logo rests at once the entrance completes. */
  restOpacity: number;
  svg: React.ReactNode;
}
...
const MARKS: Mark[] = [
  ...
  {
    depth: 2.2,
    className: "top-[8%] left-[64%] h-[52px] w-[52px] text-[#10a37f]",
  ...
  {
    depth: 2.8,
    className: "top-[38%] left-[50%] h-[44px] w-[44px] text-[#cc7b5c]",
  ...
  {
    depth: 3,
    className: "top-[12%] left-[84%] h-[46px] w-[46px] text-[#4285f4]",
  ...
  {
    depth: 1.8,
    className: "top-[85%] left-[76%] h-[38px] w-[38px] text-[#1f7a8c]",
  ...
  {
    depth: 1.4,
    className: "top-[65%] left-[45%] h-[34px] w-[34px] text-[#1a1a1a]",
...
// Entrance: each logo fades in alongside the headline (220ms base) with a
// 40ms stagger so the cluster reads as a cohesive constellation appearing,
// not five marks popping. Transform is owned by Floating's RAF; only the
// inner <svg> gets the keyframe (the wrapper carries opacity-via-class so
// the JS-controlled transform on the wrapper stays clean).
const ENTRANCE_BASE_MS = 220;
    ...
    <>
      {MARKS.map((mark, i) => (
        <FloatingElement key={i} depth={mark.depth}>
          <div
            aria-hidden="true"
            className={`pointer-events-none absolute z-10 motion-safe:animate-enter-fade max-[900px]:hidden ${mark.className}`}
            style={{
              opacity: mark.restOpacity,
              ["--enter-final-opacity" as string]: mark.restOpacity,
              animationDelay: `${ENTRANCE_BASE_MS + i * ENTRANCE_STAGGER_MS}ms`,
            }}
          >
            <svg
              viewBox="0 0 32 32"
              ...
              fill="none"
              className={`block h-full w-full ${mark.ambient}`}
            >
              {mark.svg}
            </svg>
          </div>
